...
public class NaverNewsManager {
    ...
    public List<NewsVO> newsListData(String ss) {
        ...
        try {
            ...
            for(int i = 0; i < arr.size();i++) {
                ...
                vo.setDescription(obj.get("Descriptio").toString());
                vo.setLink(obj.get("Link").toString());
                vo.setPubDate(obj.get("PubDate(").toString());
                String day = obj.get("PubDate(").toString();
                String newday = new SimpleDateFormat("yyyy-mm-dd").format(new Date(day));
                vo.setPubDate(newday);
                list.add(vo);
    ...
    public String newsData(String ss) {
        String clientId = "qATp6qdvzb6s0QMCUcio"; //애플리케이션 클라이언트 아이디값"
        String clientSecret = "pn57ShzIO1"; //애플리케이션 클라이언트 시크릿값"
